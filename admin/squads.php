...
if(isset($_GET['delete'])) {
	...
	if($CAPCLASS->check_captcha(0, $_GET['captcha_hash'])) {
		...
		$ergebnis=safe_query("SELECT upID FROM ".PREFIX."upcoming WHERE squad='$squadID' OR opponent='$squadID'");
		while($ds=mysql_fetch_array($ergebnis)) {
		...
		safe_query("DELETE FROM ".PREFIX."clanwars WHERE squad='$squadID' OR opponent='$squadID'");
...
if(isset($_GET['dead'])) {
	...
	if($CAPCLASS->check_captcha(0, $_GET['captcha_hash'])) {
		...
		if ($_GET['dead'] == 1)
		{
			safe_query("UPDATE ".PREFIX."squads SET dead = 1 WHERE squadID='$squadID'");
		}
		else if ($_GET['dead'] == 0)
		{
			safe_query("UPDATE ".PREFIX."squads SET dead = 0 WHERE squadID='$squadID'");
		}
...
if(isset($_POST['saveedit'])) {
	...
	if($CAPCLASS->check_captcha(0, $_POST['captcha_hash'])) {
		if(checkforempty(Array('name'))) {
			...
			if(isset($_POST['icon_delete']))
			{
				$query = safe_query("SELECT icon FROM ".PREFIX."squads WHERE squadID='".$id."'");
				$ds = mysql_fetch_assoc($query);
				if($ds)
				{
					if(file_exists($filepath.$ds['icon']) && !empty($ds['icon']))
					{
						unlink($filepath.$ds['icon']);
						safe_query("UPDATE ".PREFIX."squads SET icon='' WHERE squadID='".$id."'");
					}
				}
				else
					echo "Error deleting icon!";
			}
			else if($icon['name'] != "") {
				move_uploaded_file($icon['tmp_name'], $filepath.$icon['name'].".tmp");
				@chmod($filepath.$icon['name'].".tmp", 0755);
			...
			if(isset($_POST['icon_small_delete']))
			{
				$query = safe_query("SELECT icon_small FROM ".PREFIX."squads WHERE squadID='".$id."'");
				$ds = mysql_fetch_assoc($query);
				if($ds)
				{
					if(file_exists($filepath.$ds['icon_small']) && !empty($ds['icon_small']))
					{
						unlink($filepath.$ds['icon_small']);
						safe_query("UPDATE ".PREFIX."squads SET icon_small='' WHERE squadID='".$id."'");
					}
				}
				else
					echo "Error deleting icon!";
			}
			else if($icon_small['name'] != "") {
				move_uploaded_file($icon_small['tmp_name'], $filepath.$icon_small['name'].".tmp");
				@chmod($filepath.$icon_small['name'].".tmp", 0755);
				$getimg = getimagesize($filepath.$icon_small['name'].".tmp");
					$pic = '';
...
elseif($action=="edit") {
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="3">
	...
	if (!empty($ds['icon'])) echo '<td width="20%"><input name="icon_delete" type="checkbox"/> Delete?</td>';

    echo '</tr>
	 <tr>
      <td width="15%"><b>'.$_language->module['current_icon_small'].'</b></td>
      <td width="65%">'.$pic_small.'</td>';
	
	if (!empty($ds['icon_small'])) echo '<td width="20%"><input name="icon_small_delete" type="checkbox"/> Delete?</td>';
  ...
  echo'<h1>&curren; '.$_language->module['squads'].'</h1>';
	...
	$squads=safe_query("SELECT * FROM ".PREFIX."squads WHERE dead = 0 ORDER BY sort");
  ...
  echo '<h2>Active clans:</h2>
    ...
    <tr>
      ...
      <td width="30%" class="title"><b>'.$_language->module['squad_info'].'</b></td>
    ...
    $i=1;
      ...
      echo'<tr>
		...
		<input type="button" onclick="MM_goToURL(\'parent\',\'admincenter.php?site=squads&amp;action=edit&amp;squadID='.$db['squadID'].'\');return document.MM_returnValue" value="'.$_language->module['edit'].'" />
        <input type="button" onclick="MM_confirm(\''.$_language->module['really_delete'].'\', \'admincenter.php?site=squads&amp;delete=true&amp;squadID='.$db['squadID'].'&amp;captcha_hash='.$hash.'\')" value="'.$_language->module['delete'].'" />
		<input type="button" onclick="MM_confirm(\''.$_language->module['really_dead'].'\', \'admincenter.php?site=squads&amp;dead=1&amp;squadID='.$db['squadID'].'&amp;captcha_hash='.$hash.'\')" value="'.$_language->module['dead'].'" /></td>
  ...
  echo'<tr>
      <td class="td_head" colspan="5" align="right"><input type="hidden" name="captcha_hash" value="'.$hash.'" /><input type="submit" name="sortieren" value="'.$_language->module['to_sort'].'" /></td>
		...
		echo 'There are no active clans.';
	...
	if ($anzsquads)
	{
		...
		while ($db=mysql_fetch_array($squads))
		{
			...
			echo'<tr>
				...
				<input type="button" onclick="MM_confirm(\''.$_language->module['really_delete'].'\', \'admincenter.php?site=squads&amp;delete=true&amp;squadID='.$db['squadID'].'&amp;captcha_hash='.$hash.'\')" value="'.$_language->module['delete'].'" />
				<input type="button" onclick="MM_confirm(\''.$_language->module['really_alive'].'\', \'admincenter.php?site=squads&amp;dead=0&amp;squadID='.$db['squadID'].'&amp;captcha_hash='.$hash.'\')" value="'.$_language->module['alive'].'" /></td>
				<td class="'.$td.'" align="center"><select name="sort[]">';
					for($j=1; $j <= $anzsquads; $j++)
					{
						if($db['sort'] == $j)
