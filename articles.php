...
elseif($action=="show") {
	...
	if(isnewsadmin($userID)) echo'<input type="button" onclick="MM_openBrWindow(\'articles.php?action=new\',\'Articles\',\'toolbar=no,status=no,scrollbars=yes,width=800,height=600\');" value="'.$_language->module['new_article'].'" class="button1"/> ';
	echo'<input type="button" onclick="MM_goToURL(\'parent\',\'index.php?site=articles\');return document.MM_returnValue;" value="'.$_language->module['all_articles'].'" class="button1"/><br /><br />';
    ...
    <input type="button" onclick="MM_confirm(\''.$_language->module['really_delete'].'\', \'articles.php?delete=true&amp;articlesID='.$ds['articlesID'].'\');" value="'.$_language->module['delete'].'" />';
      ...
      <table cellspacing="0" cellpadding="2" align="right">
        <tr>
          ...
          <input type="submit" name="Submit" value="'.$_language->module['rate'].'" class="button1" /></td>
        </tr>
      </table>
      </form></br>';
		}
