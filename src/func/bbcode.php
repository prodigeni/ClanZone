...
function htmlnl($text){
	preg_match_all('/<(table|li|ul|ol|tr|td|dl|dt|dd|dir|menu|th|thead|caption|colgroup|col|tbody|tfoot|div|span*)[^>]*>(.*?)<\/\1>/si',$text,$matches,PREG_SET_ORDER);
...
function codereplace($content) {
	...
	for($i=0;$i<$anz;$i++){
		...
		if(strtolower($splits[$i]) == "[code]"){
			...
			if($match){
				$splits[$i] = '<div  class="code"><b>'.$_language->module['code'].':</b><div class="codeinner">';
...
function tubereplace($content) {
	...
	$bg1=BG_1;
  	 ...
  	 $content = preg_replace("#\[youtube\](.*?)\[/youtube\]#si", '<object width="425" height="350"><param name="movie" value="http://www.youtube.com/v/\\1"></param><embed src="http://www.youtube.com/v/\\1" type="application/x-shockwave-flash" wmode="opaque" width="425" height="350"></embed></object>', $content);

//vimeo 
$content = preg_replace("#\[vimeo\]([0-9]{0,})\[/vimeo\]#si","<object width=\"630\" height=\"358\"><param name=\"allowfullscreen\" value=\"true\" /><param name=\"allowscriptaccess\" value=\"always\" /><param name=\"movie\" value=\"http://www.vimeo.com/moogaloop.swf?clip_id=\\1&server=www.vimeo.com&show_title=1&show_byline=1&show_portrait=0&color=&fullscreen=1\" /><embed src=\"http://www.vimeo.com/moogaloop.swf?clip_id=\\1&server=www.vimeo.com&show_title=1&show_byline=1&show_portrait=0&color=&fullscreen=1\" type=\"application/x-shockwave-flash\" allowfullscreen=\"true\" allowscriptaccess=\"always\" width=\"630\" height=\"358\"></embed></object>", $content);

//myvideo Videos einbinden [myvideo]id[/myvideo]
	$content = preg_replace("#\[myvideo\](.*?)\[/myvideo\]#si", "<object classid=\"clsid:d27cdb6e-ae6d-11cf-96b8-444553540000\" width=\"425\" height=\"367\"> <param name=\"movie\" value=\"http://www.myvideo.de/movie/\\1\"></param><embed src=\"http://www.myvideo.de/movie/\\1\" width=\"425\" height=\"367\" type=\"application/x-shockwave-flash\"></embed></object>", $content); 

//google Videos einfügen [googlevideo]id[/googlevideo]
	$content = preg_replace("#\[googlevideo\](.*?)\[/googlevideo\]#si", "<embed style=\"width:425px; height:367px;\" id=\"VideoPlayback\" type=\"application/x-shockwave-flash\" src=\"http://video.google.com/googleplayer.swf?docId=\\1&hl=de\" flashvars=\"\"> </embed>", $content);

//wipido videos [wipido]id[/wipido]
	$content = preg_replace("#\[wipido\](.*?)\[/wipido\]#si", "<iframe src=\"http://www.wipido.com/main/video/external/\\1\" style=\"border:none; width:480px;height:320px;overflow:hidden;\" scrolling=\"no\" border=\"0\" frameborder=\"0\"></iframe>", $content);
	
//xfire [xfire][/xfire]
	$content = preg_replace("#\[xfire\](.*?)\[/xfire\]#si", "<object width=\"420\" height=\"380\"><embed src=\"http://media.xfire.com/swf/embedplayer.swf\" type=\"application/x-shockwave-flash\" allowscriptaccess=\"always\" allowfullscreen=\"true\" width=\"320\" height=\"280\" flashvars=\"videoid=\\1\"></embed></object>", $content);

//gametrailers videos [gt]id[/gt]
	$content = preg_replace("#\[gt\](.*?)\[/gt\]#si", "<embed src=\"http://www.gametrailers.com/remote_wrap.php?mid=\\1\" swLiveConnect=\"true\" name=\"gtembed\" align=\"middle\" allowScriptAccess=\"sameDomain\" allowFullScreen=\"true\" quality=\"high\" pluginspage=\"http://www.macromedia.com/go/getflashplayer\" type=\"application/x-shockwave-flash\" width=\"480\" height=\"392\"></embed>", $content);
...
function quotereplace($content) {
	...
	$content = preg_replace("#\[quote=(.*?)\]#si", "[quote][b]\\1 ".$wrote.":[/b][br]",$content);
...
function replacement($content, $bbcode=true) {
	...
	if($bbcode) {
		...
		$content = preg_replace("#\[pre\](.*?)\[/pre\]#si", "<pre class='brush: c-sharp;'>\\1</pre>",$content);
		//bbcode tables
    	...
    	$content=preg_replace("#\[php\](.*?)\[\/php\]#se","'PHP-Code:<div style=\"padding: 5px;background-color:$bg_div; width: $w_div; overflow:auto;\">							'.highlight_string(prepare_php('$1'), true).'</div>'", $content);
	}
