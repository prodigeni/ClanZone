...
if(isset($_POST['savevisitorcomment'])) {
	...
	elseif($errormsg != "" ) header("Location: ".$_POST['referer']."&error=badword&errmsg=".$errormsg."#addcomment");
	else {
		$date=time();
...
elseif(isset($_POST['saveeditcomment'])) {
	...
	if($comments_allowed) {
		...
		elseif($comments_allowed == 2) {
			...
			elseif($err=="badword") $error = $_GET['errmsg'].' '.$_language->module['error_spam'].' "'.$_GET['errmsg'].'"';
...
function checkcomment($commentquery) {
	$spamdetected = "";
	$erg=mysql_query("SELECT * from ".PREFIX."badword");
	while ($da=mysql_fetch_array($erg)) {
		if(stristr($commentquery, $da['badword'])){ 
			if($spamdetected=="") $spamdetected = $da['badword'];
			else $spamdetected .= " - ".$da['badword'];
		}
	}
	return $spamdetected;
}
