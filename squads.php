...
if($action=="show") {
	...
	if(isset($_GET['squadID']) && is_numeric($_GET['squadID']))
		$ergebnis=safe_query("SELECT * FROM ".PREFIX."squads WHERE squadID = ".$_GET['squadID']." ORDER BY sort");
	else if($_GET['all'] == 1)
		$ergebnis=safe_query("SELECT * FROM ".PREFIX."squads ORDER BY sort");
	else if($_GET['dead'] == 1)
		$ergebnis=safe_query("SELECT * FROM ".PREFIX."squads WHERE dead = 1 ORDER BY sort");
	else
		$ergebnis=safe_query("SELECT * FROM ".PREFIX."squads WHERE dead = 0 ORDER BY sort");
