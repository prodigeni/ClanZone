      ...
      <td colspan="3"><b>'.$_language->module['upcoming_clanwars'].'</b><br />&nbsp;</td>
		...
		while($squad=mysql_fetch_array($squads)) {
			...
			if(isgamesquad($squad['squadID'])) {
				...
				$n = 1;
				$ergebnis=safe_query("SELECT * FROM `".PREFIX."upcoming` WHERE type='c' AND (squad='".$squad['squadID']."' OR opponent='".$squad['squadID']."') AND date>".time()." ORDER by date");
        ...
        if($anz) {
			$clanwars .= '<table border="0" width="98%" cellpadding="2">
				<tr>
					...
					<td width="20%"><b>'.$_language->module['team1'].'</b></td>
					<td width="20%"><b>'.$_language->module['team2'].'</b></td>
				...
				while($ds=mysql_fetch_array($ergebnis)) {
					...
					$clanwars .= '<tr>
						...
						<td bgcolor="'.$bg.'"><a href="index.php?site=squads&amp;action=show&amp;squadID='.$ds['squad'].'">'.getsquadname($ds['squad']).'</a></td>
						<td bgcolor="'.$bg.'"><a href="index.php?site=squads&amp;action=show&amp;squadID='.$ds['opponent'].'">'.getsquadname($ds['opponent']).'</a></td>
