...
function forum_stats() {
	...
	// WHOISONLINE
	$guests = mysql_num_rows(safe_query("SELECT ip FROM ".PREFIX."whoisonline WHERE userID='0'"));
	$user = mysql_num_rows(safe_query("SELECT userID FROM ".PREFIX."whoisonline WHERE userID!='0'"));
...
function boardmain() {
	...
	while($dk=mysql_fetch_array($kath)) {
		...
		while($db=mysql_fetch_array($boards)) {
			...
			if($i%2) {
				$bg1=BG_1;
				$bg2='images/silver_forum/td1.jpg';
			}
			else {
				$bg1=BG_3;
				$bg2='images/silver_forum/td2.jpg';
			}
...
if(isforumadmin($userID) OR ismoderator($userID,$dt['boardID'])) {
	$action ='<input class="input" type="checkbox" name="topicID[]" value="'.$dt['topicID'].'" />';
}
		...
		$adminactions = "";
if(isforumadmin($userID) OR ismoderator($userID,$board)) {
	...
	<option value="closetopic">- '.$_language->module['close_topic'].'</option>
	<option value="opentopic">- '.$_language->module['reopen_topic'].'</option>
	<option value="deletetopics">- '.$_language->module['delete_topic'].'</option>
	</select>
