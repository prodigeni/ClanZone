...
elseif($action=="showall") {
	...
	while($ds=mysql_fetch_array($ergebnis)) {
		...
		$name=$ds['name'];
		$message=activateLinks(cleartext($ds['message'], false));
		$ip='logged';
	...
	if(isfeedbackadmin($userID)) $submit='<input class="input" type="checkbox" name="ALL" value="ALL" onclick="SelectAll(this.form);" /> '.$_language->module['select_all'].' <input type="submit" value="'.$_language->module['delete_selected'].'" />';
	else $submit='';
	echo $page_link.' '.$submit.'</form>';
...
elseif(basename($_SERVER['PHP_SELF'])!="shoutbox_content.php"){
	...
	while($ds=mysql_fetch_array($ergebnis)) {
		...
		$message=activateLinks(str_replace("&amp;amp;","&",$message));
