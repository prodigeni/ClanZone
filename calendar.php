		...
		while ($ds=mysql_fetch_array($ergebnis)) {
			if($ds['type']=="c") {
				if($ds['date']>=$start_date && $ds['date']<=$end_date) {
					...
					$opponent=getsquadname($ds['opponent']);
					...
					if(isclanmember($userID) or isanyadmin($userID)) {
						...
						if(issquadmember($userID, $ds['squad']) OR issquadmember($userID, $ds['opponent']) AND $ds['date']>time()) $announce='&#8226; <a href="index.php?site=calendar&amp;action=announce&amp;upID='.$ds['upID'].'">'.$_language->module['announce_here'].'</a>';
						else $announce = "";
...
if($action=="savewar") {
	...
	if($messages) {
		$replace = array('%date%' => date("d.m.Y", $date),
						 '%team1_hp%' => 'index.php?site=squads&action=show&squadID='.$squad,
						 '%team2_hp%' => 'index.php?site=squads&action=show&squadID='.$opponent,
						 '%team1%' => getsquadname($squad),
						 '%team2%' => getsquadname($opponent),
						 '%league_hp%' => $leaguehp,
						 '%league%' => $league,
						 '%warinfo%' => $warinfo);
		$ergebnis=safe_query("SELECT userID FROM ".PREFIX."squads_members WHERE squadID='$squad' OR squadID='$opponent'");
